...
| 特性 | 描述 |
|------|------|
| 🎯 **站点优先降级** | 先换站点再换模型，保证用户得到想要的模型 |
| ⚡ **快速失败机制** | 所有资源不可用时立即返回错误，不浪费时间 |
| 🔄 **熔断器半开状态** | 冷却期后渐进式恢复，避免雪崩 |
| 🎲 **冷却时间抖动** | 防止惊群效应，分散流量 |
| 📊 **模型自动降级** | 负载过高时自动切换到可用模型 |
| 🎚️ **智能降级策略** | 支持 site_first（站点优先）和 model_first（模型优先）|
| 🔍 **主站紧急检查** | 不在主站时使用更短检查间隔，快速恢复 |
| 📈 **指数退避机制** | 失败后冷却时间逐步增加，避免频繁重试 |
...
# v2.1 高可用配置
SITE_MANAGER_ENABLED=true              # 站点管理器开关
SITE_MAX_CONSECUTIVE_FAILS=5           # 连续失败次数触发切换
SITE_COOLDOWN_SECONDS=60               # 站点冷却时间
MODEL_DEGRADATION_ENABLED=true         # 模型自动降级
DEGRADATION_STRATEGY=site_first        # 降级策略（site_first/model_first）
FAST_FAIL_ENABLED=true                 # 快速失败
HALF_OPEN_ENABLED=true                 # 熔断器半开状态
COOLDOWN_JITTER_ENABLED=true           # 冷却时间抖动
```
